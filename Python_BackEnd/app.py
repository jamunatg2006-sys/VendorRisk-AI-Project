...
from logic.scoring_engine import RiskAggregator, RiskBenchmarks
...
def analyze():
    ...
    # ── Response ──────────────────────────────────────────────────────────
    if final_report["risk_level"] in ["High", "Critical"]:
        try:
            alert_response = requests.post(
                ...
                json={
                    ...
                    "message": f"Vendor {vendor_name} scored {final_report['risk_score']}/100 ({final_report['risk_level']} risk)",
                    "details": final_report
                },
                timeout=5
            )
            if alert_response.ok:
                print(f"📧 Alert triggered for {vendor_name}")
            else:
                print(f"⚠️ Alert endpoint returned {alert_response.status_code}: {alert_response.text}")
        except Exception as e:
